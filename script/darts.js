...
/*
 * 設計
 * 1本目アウト → {outFlag : true, [19T]}
 * 2本目アウト → {outFlag : true, [19S, SB]}
 * 3本目アウト → {outFlag : true, [19s, 20s, SB]}
 * ノーアウト  → {outFlag : false, [NO OUT!!]}
 */
 ...
 * @param {*} hitArea 
 */
function calc2hon(zanPoint, hitPoint, hitArea) {
	...
	// バースト
	// ==============================
	if (isBurst(zan, hitArea, hitPoint)) {
		return [BURST];
	}
 ...
 * @param {*} zan 
 */
function isBurst(zan, hitArea, hitPoint) {
	if (zan < 0) {
		return true;
	}
	if (zan == 1 && !singleOutFlag) {
	...
	if ( isOut(zan, hitArea, hitPoint) && zan == 0) {
		// バースト
		return true;
	}
	return false;
 ...
 * @param {*} zan 
 */
function isOut(zan, hitArea, hitPoint) {
	if ( zan != 0 ) {
		return false;
	}
	if (masterOutFlag && hitArea == HIT_AREA_TRIPLE) {
